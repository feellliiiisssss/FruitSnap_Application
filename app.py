...
def load_model():
    ...
    model_dir = 'model'
    model_filename = 'FruitSnap_model.h5'
    ...
    if not os.path.isfile(model_path):
        tf.keras.utils.get_file(fname=model_filename, origin=url_path, cache_dir=model_dir, cache_subdir='.')
...
def predict_image(current_user):
    ...
    try :
        ...
        add_data('history', current_user["username"], { 'predicted': class_name, 'timestamp': datetime.datetime.utcnow()})
	...
	# app.logger.error(f"Error occurred: {err}")
        # Return a JSON response with the error message and a 401 status code
